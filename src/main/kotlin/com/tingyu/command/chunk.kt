    ...
    val chunk = mainCommand {
        ...
        literal("generate") {
            ...
            """.trimIndent()
                ...
                val coordText = Components.text("§a点击复制坐标 §6[$startX, 0, $startZ] §7→ §6[$endX, 0, $endZ]")
                    .clickCopyToClipboard("$startX $startZ $endX $endZ")
                    .hoverText("§e点击复制坐标")
                    .sendTo(adaptPlayer(sender))  // 直接链式调用发送
                createPillars(corners, sender)
            }
        }
